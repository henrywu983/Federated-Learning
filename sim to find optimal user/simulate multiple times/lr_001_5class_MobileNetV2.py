...
transform = transforms.Compose([
    transforms.Resize(224),  # Resize for pretrained MobileNetV2
...
class MobileNetV2Custom(nn.Module):
    def __init__(self, num_classes=10, width_mult=1.0, train_dense_only=False, weights='IMAGENET1K_V1'):
        super(MobileNetV2Custom, self).__init__()
        
        # Load MobileNetV2 with a reduced width multiplier
        self.base_model = torchvision.models.mobilenet_v2(width_mult=width_mult, weights=weights)
        
        # Modify the first conv layer to handle 32x32 input (CIFAR-10)
        # Original: 3 input channels, 32 output channels, stride=2, kernel=3
        # Adjusted: stride=1 to avoid excessive downsampling
        self.base_model.features[0][0] = nn.Conv2d(3, int(32 * width_mult), kernel_size=3, stride=1, padding=1, bias=False)
        ...
        in_features = self.base_model.classifier[1].in_features  # Typically 1280 for width_mult=1.0, scaled by width_mult
        self.base_model.classifier = nn.Sequential(
            nn.Dropout(0.2),  # Default dropout in MobileNetV2
...
def calculate_gradient_difference(w_before, w_after):
    return {k: w_before[k] - w_after[k] for k in w_before.keys()}
...
for run in range(num_runs):
    ...
    for seed_index, seed in enumerate(seeds_for_avg):
        ...
        model = MobileNetV2Custom(num_classes=10, width_mult=1.0, weights='IMAGENET1K_V1').to(device)
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9)

        w_before_train = model.state_dict()  # Full state dictionary

        memory_matrix = {user_id: [torch.zeros_like(param).to(device) for param in w_before_train.values()] for user_id in range(num_users)}
        ...
        for timeframe in range(num_timeframes):
            ...
            sparse_gradient = {user_id: [torch.zeros_like(param).to(device) for param in w_before_train.values()] for user_id in range(num_users)}
            ...
            for user_id in range(num_users):
                ...
                model.load_state_dict(w_before_train)
                torch.cuda.empty_cache()
                ...
                gradient_diff_memory = {k: gradient_diff[k] + memory_matrix[user_id][j] for j, k in enumerate(w_before_train.keys())}

                if use_memory_matrix:
                    sparse_gradient[user_id] = top_k_sparsificate_model_weights(list(gradient_diff_memory.values()), fraction[0])
                else:
                    sparse_gradient[user_id] = top_k_sparsificate_model_weights(list(gradient_diff.values()), fraction[0])
                for j, k in enumerate(w_before_train.keys()):
                    memory_matrix[user_id][j] = (gamma_momentum[0] * memory_matrix[user_id][j]
                                 + gradient_diff_memory[k] - sparse_gradient[user_id][j])

                gradient_l2_norm = torch.norm(torch.stack([torch.norm(g.float()) for g in gradient_diff.values()])).item()
                gradient_l2_norm_memory = torch.norm(torch.stack([torch.norm(g.float()) for g in gradient_diff_memory.values()])).item()
            ...
            if num_distinct_users > 0:
                sum_terms_dict = {k: sum_terms[i] for i, k in enumerate(w_before_train.keys())}
                new_weights = {k: w_before_train[k] + sum_terms_dict[k] / num_distinct_users for k in w_before_train.keys()}
            else:
                new_weights = {k: param.clone() for k, param in w_before_train.items()}
            model.load_state_dict(new_weights)
            ...
            weight_update = [new_weights[k] - w_before_train[k] for k in w_before_train.keys()]
